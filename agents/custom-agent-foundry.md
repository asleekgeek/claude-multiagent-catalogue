...
> Ported from awesome-copilot/agents/custom-agent-foundry.agent.md
...
- **Deployment agents**: Include `['run_in_terminal', 'create_and_run_task', 'project linters/type-checkers (`npm run lint`, `tsc --noEmit`, `ruff check`, `mypy`) via `Bash`']`
...
- Reference tools in body: `toolName` (e.g., `githubRepo`)
...
- get_errors → bash linters/type-checkers
