...
> Ported from awesome-copilot/agents/Thinking-Beast-Mode.agent.md
...
Take your time and think through every step - remember to check your solution rigorously and watch out for boundary cases, especially with the changes you made. Use the Sequential Thinking MCP server if available (`mcp__sequential-thinking__sequentialthinking`). Your solution must be perfect. If not, continue working on it. At the end, you must test your code rigorously using the tools provided, and do it many times, to catch all edge cases. If it is not robust, iterate more and make it perfect. Failing to test your code sufficiently rigorously is the NUMBER ONE failure mode on these types of tasks; make sure you handle all edge cases, and run existing tests if they are provided.
...
1. **🧠 Quantum Thinking Initialization:** Use the Sequential Thinking MCP server (`mcp__sequential-thinking__sequentialthinking`) for deep cognitive architecture activation
...
- Use the `WebFetch` tool to search for information.
...
- Run the project's linters and type-checkers directly via `Bash` (e.g. `npm run lint`, `tsc --noEmit`, `ruff check`, `mypy`) to identify and report issues. Where an LSP integration is available, use it.
...
You must use the Sequential Thinking MCP server (`mcp__sequential-thinking__sequentialthinking`) for every problem, implementing a multi-layered cognitive architecture:
...
Three Copilot-specific tool names were mapped to Claude Code equivalents (per `copilot-to-claude-multiagent/references/tool-mapping.md` and `references/conversion-patterns.md §3, §5`):

- `fetch_webpage` → `WebFetch` (4 occurrences).
- `sequential_thinking` → `mcp__sequential-thinking__sequentialthinking` (the Sequential Thinking MCP server). Falls back to disciplined inline reasoning if the MCP server is not configured.
- `get_errors` (§8 Debugging) → project linters/type-checkers via `Bash` (e.g. `npm run lint`, `tsc --noEmit`, `ruff check`, `mypy`); LSP tool where integrated.

Frontmatter normalisation: added `model: claude-opus-4-6` (heavy tier — this agent's prompt structure signals deep reasoning).

Filename normalisation: `Thinking-Beast-Mode.agent.md` → `thinking-beast-mode.md`.
