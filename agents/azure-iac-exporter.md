...
description: >
  Export existing Azure resources to Infrastructure as Code templates via Azure Resource
  Graph analysis, Azure Resource Manager API calls, and azure-iac-generator integration.
  Use this skill when the user asks to export, convert, migrate, or extract existing Azure
  resources to IaC templates (Bicep, ARM Templates, Terraform, Pulumi).
model: claude-sonnet-4-6
---

> Ported from awesome-copilot/agents/azure-iac-exporter.agent.md
...
9. **IaC Code Generation**: Call azure-iac-generator subagent to generate target format code:
   ...
   - Action: Call `#the Task tool` with `agentName="azure-iac-generator"`
   - Example payload:
     ```json
     {
...
### Tool Usage Patterns
- Use `read` to analyze source IaC files and understand current structure
- Use `search` to find related infrastructure components across projects and locate IaC files
- Use `execute` for format-specific CLI tools (az bicep, terraform, pulumi) when needed for source analysis
- Use `web` to research source format syntax and extract requirements when needed
- Use `todo` to track migration progress for complex multi-file projects
- **IaC Code Generation**: Use `#the Task tool` to call azure-iac-generator with comprehensive infrastructure requirements for target format generation with format-specific validation

**Step 1: Smart Resource Discovery (Azure Resource Graph)**
- Use `ms-azuretools.vscode-azure-github-copilot/azure_query_azure_resource_graph` with queries like:
...
- Once resource is identified, use `ms-azuretools.vscode-azure-github-copilot/azure_query_azure_resource_graph` to fetch detailed resource properties and control plane metadata
...
- Call appropriate Azure MCP tools based on specific resource type for data plane metadata collection:
  - `azure-mcp/storage` for Storage Accounts data plane metadata and configuration insights
  - `azure-mcp/keyvault` for Key Vault data plane metadata and policy analysis
  - `azure-mcp/aks` for AKS cluster data plane metadata and configuration details
  - `azure-mcp/appservice` for App Service data plane metadata and application analysis
  - `azure-mcp/cosmos` for Cosmos DB data plane metadata and database properties
  - `azure-mcp/postgres` for PostgreSQL data plane metadata and configuration analysis
  - `azure-mcp/mysql` for MySQL data plane metadata and database settings
  - `azure-mcp/functionapp` for Function Apps data plane metadata
  - `azure-mcp/redis` for Redis Cache data plane metadata
  - And other resource-specific Azure MCP tools as needed

**Step 4: User-Configured Properties Only (Az Rest API)**
- Use `execute` with `az rest` commands to collect only user-configured data plane properties:
...
- Use `read` to analyze existing project structure and naming conventions
- Use `search` to understand existing IaC templates and patterns

**Step 7: IaC Code Generation**
- Use `#the Task tool` to call azure-iac-generator with filtered resource analysis (user-configured properties only) and infrastructure requirements for format-specific template generation
...
10. **IaC Code Generation**: Call azure-iac-generator subagent with analysis summary and infrastructure requirements:
    ...
    - Call `#the Task tool` with `agentName="azure-iac-generator"` providing:
...
Copilot-specific tool names mapped to Claude Code equivalents (per `copilot-to-claude-multiagent/references/conversion-patterns.md` §3/§5):

- #tool:* prefix stripped
- runSubagent → Task

Model coerced from upstream `'Claude Sonnet 4.5'` → `claude-sonnet-4-6` per `references/model-mapping.md`.
