# gem-team — External Plugin

> External plugin (no local content in this catalogue). Source: https://github.com/mubaidr/gem-team
> Author: [mubaidr](https://github.com/mubaidr) | Version: 1.24.0 | License: Apache-2.0 | Converted: 2026-05-19

Self-Learning Multi-agent orchestration harness for spec-driven development and automated verification.

## Installation

This plugin is hosted in an external repository. There are two install paths:

### Via APM (if maintained by upstream)
...
See the upstream README at https://github.com/mubaidr/gem-team for the canonical install procedure and agent roster.

## Claude Code Notes

This plugin's content lives in its own repository, not in this catalogue. Only the manifest is mirrored here so `install.sh --list` and `CATALOGUE.md` can surface its existence. The upstream maintainers are responsible for keeping it Claude Code-compatible; this entry will not drift if upstream evolves.
