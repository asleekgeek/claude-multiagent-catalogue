...
description: >
  Test suite fixer and verifier for React 16/17 → 18.3.1 migration. Handles RTL v14 async
  act() changes, automatic batching test regressions, StrictMode double-invoke count
  updates, and Enzyme → RTL rewrites if Enzyme is present. Loops until zero test failures.
  Invoked as subagent by react18-commander.
model: claude-sonnet-4-6
---

> Ported from awesome-copilot/plugins/react18-upgrade/agents/react18-test-guardian.md
...
```
memory read repository "react18-test-state"
```

Write after each file and each run:

```
memory write repository "react18-test-state" "file:[name]:status:fixed"
memory write repository "react18-test-state" "run-[N]:failures:[count]"
```
...
```
memory write repository "react18-test-state" "complete:0-failures:all-green"
```
