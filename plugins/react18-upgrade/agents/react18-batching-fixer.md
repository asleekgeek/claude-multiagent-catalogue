...
description: >
  Automatic batching regression specialist. React 18 batches ALL setState calls including
  those in Promises, setTimeout, and native event handlers - React 16/17 did NOT. Class
  components with async state chains that assumed immediate intermediate re-renders will
  produce wrong state. This agent finds every vulnerable pattern and fixes with flushSync
  where semantically required.
model: claude-sonnet-4-6
---

> Ported from awesome-copilot/plugins/react18-upgrade/agents/react18-batching-fixer.md
...
```
memory read repository "react18-batching-progress"
```

Write checkpoints:

```
memory write repository "react18-batching-progress" "file:[name]:status:[fixed|clean]"
```
...
```
memory write repository "react18-batching-progress" "complete:flushSync-insertions:[N]"
```
