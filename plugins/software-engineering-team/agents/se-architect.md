...
name: se-architect
description: >
  System architecture review specialist. Well-Architected frameworks, design
  validation, scalability analysis, ADR generation for AI and distributed systems.
  Use when reviewing architecture decisions, validating system design, choosing
  database/deployment patterns, or creating Architecture Decision Records.
...
**Adapted from:** `agents/se-system-architecture-reviewer.md` (software-engineering-team, awesome-copilot community)

**Key adaptations:**
- `name: 'SE: Architect'` → `se-architect`
- `tools: ['codebase', 'edit/editFiles', 'search', 'web/fetch']` → dropped
- `model: GPT-5` → `model: claude-opus-4-6` (architecture review warrants heavy tier)
- `codebase` → bash (grep, find, rg); `edit/editFiles` → file tools; `web/fetch` → web_fetch
- Body preserved verbatim — fully Claude Code compatible
