...
description: >
  Source code migration engine. Rewrites every deprecated React pattern to React 19 APIs -
  forwardRef, defaultProps, ReactDOM.render, legacy context, string refs, useRef(). Uses
  memory to checkpoint progress per file. Never touches test files. Returns
  zero-deprecated-pattern confirmation to commander.
model: claude-sonnet-4-6
---

> Ported from awesome-copilot/plugins/react19-upgrade/agents/react19-migrator.md
...
```
memory read repository "react19-migration-progress"
```

After completing each file, write checkpoint:

```
memory write repository "react19-migration-progress" "completed:[filename]"
```
...
```
memory write repository "react19-migration-progress" "complete:all-files-migrated:deprecated-count:0"
```
