...
description: >
  Master orchestrator for React 19 migration. Invokes specialist subagents in sequence -
  auditor, dep-surgeon, migrator, test-guardian - and gates advancement between steps.
  Uses memory to track migration state across the pipeline. Zero tolerance for incomplete
  migrations.
model: claude-sonnet-4-6
---

> Ported from awesome-copilot/plugins/react19-upgrade/agents/react19-commander.md
...
```
memory read repository "react19-migration-state"
```

Write memory after each gate passes:

```
memory write repository "react19-migration-state" "[state JSON]"
```
...
Execute each phase by invoking the appropriate subagent with `agent`. Pass the full context needed. Do NOT advance until the gate condition is confirmed.
...
```
memory write repository "react19-migration-state" {"phase":"deps","auditComplete":true,...}
```
...
```
memory write repository "react19-migration-state" {"phase":"migrate","depsComplete":true,"reactVersion":"[confirmed version]",...}
```
...
```
memory write repository "react19-migration-state" {"phase":"tests","migrateComplete":true,...}
```
...
```
memory write repository "react19-migration-state" {"phase":"done","testsComplete":true,"failedTests":0,...}
```
