...
description: >
  Dependency upgrade specialist. Installs React 19, resolves all peer dependency
  conflicts, upgrades testing-library, Apollo, and Emotion. Uses memory to log each
  upgrade step. Returns GO/NO-GO to the commander. Invoked as a subagent by
  react19-commander.
model: claude-sonnet-4-6
---

> Ported from awesome-copilot/plugins/react19-upgrade/agents/react19-dep-surgeon.md
...
```
memory read repository "react19-deps-state"
```

Write state after each step:

```
memory write repository "react19-deps-state" "step3-complete:apollo-upgraded"
```
