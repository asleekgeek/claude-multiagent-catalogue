# governance-audit — Claude Code Hook Bundle

> Ported from `awesome-copilot/hooks/governance-audit` per `copilot-to-claude-multiagent/references/conversion-patterns.md` §14
...
```bash
/path/to/claude-multiagent-catalogue/install.sh install-hook governance-audit
```

The installer copies the scripts into `.claude/hooks/governance-audit/` and **prints** the `claude-settings.json` fragment below for you to merge into your `~/.claude/settings.json` deliberately — it does NOT auto-merge (hook commands execute on session events; silent overwrite is too dangerous).
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".claude/hooks/governance-audit/audit-session-start.sh",
            "timeout": 5
          }
        ]
      }
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".claude/hooks/governance-audit/audit-session-end.sh",
            "timeout": 5
          }
        ]
      }
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".claude/hooks/governance-audit/audit-prompt.sh",
            "timeout": 10
          }
        ]
      }
