# session-auto-commit — Claude Code Hook Bundle

> Ported from `awesome-copilot/hooks/session-auto-commit` per `copilot-to-claude-multiagent/references/conversion-patterns.md` §14

**Events covered (Claude Code):** Stop

**Upstream events:** sessionEnd

## Installation

```bash
/path/to/claude-multiagent-catalogue/install.sh install-hook session-auto-commit
```

The installer copies the scripts into `.claude/hooks/session-auto-commit/` and **prints** the `claude-settings.json` fragment below for you to merge into your `~/.claude/settings.json` deliberately — it does NOT auto-merge (hook commands execute on session events; silent overwrite is too dangerous).
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".claude/hooks/session-auto-commit/auto-commit.sh",
            "timeout": 30
          }
        ]
      }
